...
#echo "************************************"
    ...
    GROUP_A_PERC_B=20 #Double
    GROUP_A_PERC_C=20 #Triple
    GROUP_A_PERC_D=40 #Multi Cabinet
...
NUMOF_RBS6K_MULTI_CABINET=`expr $NUMOF_RBS6K_MULTI_CABINET + \( $MULTICABINET - $MOD \)`
    ...
    NUMOF_RBS6K_MULTI_CABINET_TRUE=`expr \( $NUMOF_RBS6K_MULTI_CABINET \* $GROUP_C_PERC_A \) / 100`
     echo "NUMBER OF MULTI CABINETID!!!!!!!="$NUMOF_RBS6K_MULTI_CABINET
    #MultiCabinet
...
#     CABID="CABID_"$ID
   ...
   # echo "     sharedCabinetIdentifier String "$RBSCOUNT >> $MOSCRIPT$MOFILEEXTENSION
    ...
    ID=`expr $ID + $INCREMENT`
    CABID="CABID_"$ID
if ([ "$RNCCOUNT" -ge 1 ] && [ "$RNCCOUNT" -le 9 ])
   then
   echo 'CREATE' >> $MOSCRIPT$MOFILEEXTENSION
    echo '(' >> $MOSCRIPT$MOFILEEXTENSION
    echo '  parent "ManagedElement=1,Equipment=1"' >> $MOSCRIPT$MOFILEEXTENSION
   ...
   # echo "     sharedCabinetIdentifier String "$RBSCOUNT >> $MOSCRIPT$MOFILEEXTENSION
    ...
    echo 'SET' >> $MOSCRIPT$MOFILEEXTENSION
    echo '(' >> $MOSCRIPT$MOFILEEXTENSION
    echo "  mo \"ManagedElement=1,Equipment=1,Subrack=1,Slot=$ZCOUNT,PlugInUnit=1\"" >> $MOSCRIPT$MOFILEEXTENSION
    echo '   exception none' >> $MOSCRIPT$MOFILEEXTENSION
    echo '   nrOfAttributes 1' >> $MOSCRIPT$MOFILEEXTENSION
    echo "   positionRef Ref \"ManagedElement=1,Equipment=1,Cabinet=$CABID\"" >> $MOSCRIPT$MOFILEEXTENSION
    echo ')' >> $MOSCRIPT$MOFILEEXTENSION
