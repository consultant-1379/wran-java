...
do
  ...
  MOFILEEXTENSION="__"$MOFILECOUNT".mo"
   ...
   echo "**** $RNCNAME$RBSNAME$COUNT ****" 
    ...
    IPINTERFACE01_networkPrefixLength=30
    IPINTERFACE01_vLan="true"
    IPINTERFACE01_vid=100

    IPINTERFACE02_networkPrefixLength=$IPINTERFACE01_networkPrefixLength
    IPINTERFACE02_vLan="true"
    IPINTERFACE02_vid=900
    ...
    echo "IPINTERFACE02_networkPrefixLength=$IPINTERFACE02_networkPrefixLength"
    echo "IPINTERFACE02_vLan=$IPINTERFACE02_vLan"
    echo "IPINTERFACE02_vid=$IPINTERFACE02_vid"
...
MOSCT
     ...
     do
       ...
       then
        ...
        if [ $VAR -eq 3 ] || [ $VAR -eq 4 ] 
        then
          echo "       vlanMembership Array Struct 1" >> $MOSCRIPT$MOFILEEXTENSION
          echo "         nrOfElements 2" >> $MOSCRIPT$MOFILEEXTENSION
          echo "           egressUntag Boolean $ETHERNETSWITCHPORT_egressUntag" >> $MOSCRIPT$MOFILEEXTENSION
          echo "           vid Integer $ETHERNETSWITCHPORT_vid " >> $MOSCRIPT$MOFILEEXTENSION
        fi

        if [ $VAR -eq 6 ] || [ $VAR -eq 7 ]
        then
          echo "       vlanMembership Array Struct 2" >> $MOSCRIPT$MOFILEEXTENSION
          echo "         nrOfElements 2" >> $MOSCRIPT$MOFILEEXTENSION
          echo "           egressUntag Boolean $ETHERNETSWITCHPORT_egressUntag" >> $MOSCRIPT$MOFILEEXTENSION
          echo "           vid Integer $ETHERNETSWITCHPORT_vid " >> $MOSCRIPT$MOFILEEXTENSION
     ...
     cat >> $MOSCRIPT$MOFILEEXTENSION << MOSCT
       ...
       (
         parent "ManagedElement=1,Equipment=1,Subrack=1,Slot=ET-MFX,PlugInUnit=1,ExchangeTerminalIp=1,InternalEthernetPort=1"
          ...
          exception none
          nrOfAttributes 3
           "vLan" Boolean "$IPINTERFACE02_vLan"
           "vid" Integer $IPINTERFACE02_vid
           "vlanRef" Ref "null"
       )
MOSCT
    ...
    (
      parent "ManagedElement=1,IpOam=1,Ip=1"
       ...
       nrOfAttributes 6
         ...
         ipInterfaceMoRef Ref "ManagedElement=1,Equipment=1,Subrack=1,Slot=ET-MFX,PlugInUnit=1,ExchangeTerminalIp=1,InternalEthernetPort=1,IpInterface=2" 
    )
MOSCT
...
MOSCT
     ...
     PRINT_IPV4ADDRESS="YES"
     for TEMPRNCID in $RBSNONIPV4ADDRESSES
     do
        if [ $RNCIDCOUNT -eq $TEMPRNCID ]
        then
           PRINT_IPV4ADDRESS="NO"
        fi
     done

     if [ $PRINT_IPV4ADDRESS == "YES" ]
     then
          echo "ipv4Addresses Array String 1" >> $MOSCRIPT$MOFILEEXTENSION
          echo  "${IPHOSTLINK_ipv4Addresses[0]}" >> $MOSCRIPT$MOFILEEXTENSION
      fi
   ...
   cat >> $MOSCRIPT$MOFILEEXTENSION << MOSCT
    ...
    SET
    (
      mo "ManagedElement=1,Equipment=1,Subrack=1,Slot=ET-MFX,PlugInUnit=1,ExchangeTerminalIp=1,InternalEthernetPort=1,IpInterface=2"
       identity 1
       exception none
       nrOfAttributes 1
         userLabel String ICON
         ...
         vLan Boolean $IPINTERFACE02_vLan
         vid Integer $IPINTERFACE02_vid
         networkPrefixLength Integer $IPINTERFACE02_networkPrefixLength
         defaultRouter0 String "$IPINTERFACE02_defaultRouter0"
         subnet String "$IPINTERFACE02_subnet"
    )
