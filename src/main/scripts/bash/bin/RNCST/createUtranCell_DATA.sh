...
"1450,256-1,54-3,65-6,30-9,31-12;"\
"2305,10-1,110-3,80-6,81-9,63-12;"\
"3021,516-1,60-3,70-6,85-9,95-12;"\
"5301,768-1,15-3,13-6,90-9,300-12;"
...
#    echo "RNCID="$RNCCOUNT",CI="$CID",USERLABEL="$UTRANCELLID",FDDARFCN="$UARFCNDL",SCRCODE="$PRIMSCRCODE",PRIMARYCPICHPOWER="$PRIMARYCPICHPOWER
#    echo -n ",QQUALMIN="$QQUALMIN",LAC="$LA"'QRXLEVMIN="$QRXLEVMIN",USEDFREQTHRESH2DECNO="$USEDFREQTHRESH2DECNO
#    echo -n ",USEDFREQTHRESH2DRSCP="$USEDFREQTHRESH2DRSCP",SRATSEARCH="$SRATSEARCH
#    echo ""


  elif [ $OUTPUT == "WRAN" ]
  then
    echo "ROWID=$CID;EXTUCFDDID=$UTRANCELLID;MUCID=$UTRANCELLID;USERLABEL=$UTRANCELLID;LAC=$LA;PCID=$PRIMSCRCODE;CID=$CID;RA=?"
  fi
