...
# Purpose     : To dd GPEH license mo for RNC
# Date        : 16 05 2017
# Who         : Harish Dunga
###########################################
# Ver2        : Modified for ENM
# Purpose     : To set motype Gpeh feature state to deactivated
# Date        : 25 07 2017
# Who         : Anusha Chitram
...
echo '   userLabel String '$RNCCOUNT >> $MOSCRIPT
echo ')' >> $MOSCRIPT
...
if [ "$RNCCOUNT" -ge 1 ] && [ "$RNCCOUNT" -le 2 ]
then
  ATM=1
  IP=0
else
...
do
  ...
  echo '  remoteCpIpAddress1 String "1.1.1.'$IUBCOUNT'"' >> $MOSCRIPT	
  echo ')' >> $MOSCRIPT
...
(
    ...
    "fingerprint" String $RNCNAME"_fp" 
...
(
    ...
    moType RncFeature
    identity RemoteDriveTest
    exception none
    nrOfAttributes 7
    "reservedBy" Array Ref 0
    "RncFeatureId" String "2"
...
cat "$SIMDIR/mo/$RNCDIR/ms.mo" >> $MOSCRIPT
#########################################
...
cat "$SIMDIR/mo/$RNCDIR/upgrademo.mo" >> $MOSCRIPT
...
./1400createIpAtmLink.sh $SIMNAME $ENV $RNCIDCOUNT
./1380createTNMOs.sh $SIMNAME $ENV $RNCIDCOUNT
...
##NEW CHANGES AUGUST 2016
##AUTHOR : XRAMJAG



##Creation of LOGM sub MOs

cat >> $MOSCRIPT << MOSCT

CREATE
(
parent "ComTop:ManagedElement=$RNCNAME,ComTop:SystemFunctions=1,RcsLogM:LogM=1"
moType RcsLogM:Log
identity AiLog
    exception none
    nrOfAttributes 3
    ...
    "progressReport" Struct
        ...
        "timeActionCompleted" String ""
        "timeOfLastStatusUpdate" String ""

    "severityFilter" Array Integer 1
         0
)
...
###creation of RDT and UeRalTimeTrace MOs 
...
(
    parent "ManagedElement=1,SystemFunctions=1,Licensing=1"
    moType RncFeature
    identity UeRealTimeTrace
    exception none
    nrOfAttributes 7
    "reservedBy" Array Ref 0
    "RncFeatureId" String "3"
    "featureState" Integer 1
    "licenseState" Integer 1
    "serviceState" Integer 0
    "keyId" String ""
