...
package com.ericsson.ci.simnet.wran_java.geocalc;
...
public class BoundingArea {
	...
	public BoundingArea(Point northEast, Point southWest) {
		...
		southEast = new Point(new DegreeCoordinate(southWest.getLatitude()),
				new DegreeCoordinate(northEast.getLongitude()));
		northWest = new Point(new DegreeCoordinate(northEast.getLatitude()),
				new DegreeCoordinate(southWest.getLongitude()));
	}
	...
	public String toString() {
		return "BoundingArea{" + "northEast=" + northEast + ", southWest="
				+ southWest + '}';
	}

	public boolean isContainedWithin(Point point) {
		boolean predicate1 = point.latitude >= this.southWest.latitude
		...
		if (southWest.longitude > northEast.longitude) { // area is going across
															// the max/min
															// longitude
															// boundaries (ie.
															// sort of back of
			...
			// in one or the other.
			boolean predicate3 = point.longitude <= northEast.longitude
					&& point.longitude >= -180;
			boolean predicate4 = point.longitude >= southWest.longitude
					&& point.longitude <= 180;

			predicate2 = predicate3 || predicate4;
		} else {
			predicate2 = point.longitude >= southWest.longitude
	...
	public boolean equals(Object obj) {
		...
		if (this.northEast != other.northEast
				&& (this.northEast == null || !this.northEast
						.equals(other.northEast))) {
			return false;
		}
		if (this.southWest != other.southWest
				&& (this.southWest == null || !this.southWest
						.equals(other.southWest))) {
			return false;
		}
		return true;
	}
