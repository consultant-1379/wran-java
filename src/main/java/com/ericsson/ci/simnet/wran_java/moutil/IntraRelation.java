package com.ericsson.ci.simnet.wran_java.moutil;
...
public class IntraRelation {
	...
	public boolean createIntraRelation(Wran w) {
		...
		double targetTotalNumOfIntraRelation =(double) ((DOMParser
				.getAttributeInt("TOTAL_UTRAN_RELATION")
				 * DOMParser
				.getAttributeInt("PERC_INTRA_RELATION")) / 100);
		
		int targetTotalNumOfIntraRelations=(int) targetTotalNumOfIntraRelation;
	...
	public void createIntraRelationsinRnc(Rnc rnc2, int numOfIntraRelations,
			int startNumOfRelations, int endNumOfRelations) {
		...
		if (rnc2.getTotalNumOfIntraRelations() + startNumOfRelations < endNumOfRelations
