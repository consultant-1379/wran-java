...
public class EarthCalc {
	...
	public static double getDistance(Point standPoint, Point forePoint) {
		...
		double diffLongitudes = toRadians(abs(forePoint.getLongitude()
				- standPoint.getLongitude()));
		double slat = toRadians(standPoint.getLatitude());
		double flat = toRadians(forePoint.getLatitude());

		// spherical law of cosines
		double c = acos((sin(slat) * sin(flat))
				+ (cos(slat) * cos(flat) * cos(diffLongitudes)));

		// Vincenty formula
		// double c = sqrt(pow(cos(flat) * sin(diffLongitudes), 2d) +
		// pow(cos(slat) * sin(flat) - sin(slat) * cos(flat) *
		// cos(diffLongitudes), 2d));
		// c = c / (sin(slat) * sin(flat) + cos(slat) * cos(flat) *
		// cos(diffLongitudes));
		// c = atan(c);
	 ...
	 */
	public static Point pointRadialDistance(Point standPoint, double bearing,
			double distance) {
